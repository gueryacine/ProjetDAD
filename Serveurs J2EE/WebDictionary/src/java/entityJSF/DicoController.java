...
public class DicoController implements Serializable {
    ...
    @EJB
    private entityBean.DicoFacade ejbFacade;
    ...
    private DicoFacade getFacade() {
     ...
     public String search(){
        ...
        if(current.getMot() != null){
            //Création temp pour stocker résultat
            Collection<Dico> temp = new ArrayList<>();
            items = new ListDataModel(getFacade().findAll());
            List<Dico> wordList = (List<Dico> ) items.getWrappedData();

            for(int i = 0; i < wordList.size(); i++){
                if(wordList.get(i).getMot().contains(current.getMot()))
                {
